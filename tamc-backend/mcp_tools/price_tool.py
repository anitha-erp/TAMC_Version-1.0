...
async def predict_prices(request: PredictionRequest):
    ...
    if specific_variant:
        specific_variant_lower = specific_variant.lower()
        matching_variants = [v for v in variants if specific_variant_lower in v.lower()]
    ...
    weather_forecast_list = predictor.weather_fetcher.get_weather_forecast(market, days=days_to_fetch)
    ...
    for variant in variants:
        ...
        for i, p in enumerate(baseline_preds):
            ...
            if i < len(weather_forecast_list):
                ...
                total_multiplier = (
                    1 + adjustments['weather_adj'] + 
                    adjustments['disease_adj'] + 
                ...
                forecasts.append(PriceForecast(
                    ...
                    weather_adjustment=round(adjustments['weather_adj'] * 100, 2),
                    disease_risk=round(adjustments['disease_risk'], 2),
        ...
        # Create sentiment info for response
    ...
    return PredictionResponse(
        ...
        message=f"Successfully generated forecasts for {len(variants)} variant(s) with sentiment analysis"
    )
